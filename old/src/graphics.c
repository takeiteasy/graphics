...
#if defined(SGL_ENABLE_CHROMA_KEY) && defined(BLIT_CHROMA_KEY)
...
#warning FIXME: Only works for multiples ^2 images
    ...
    case 0: // RGB
      switch (info.bits) { // BPP
        case 1:
          ...
          sgl_destroy(s);
        case 4:
#define TEST_J j = (i - (i % info.width)) + (info.width - (i % info.width) - 1)
          for (int j = 0, i = (sz - 1); i != -1; i--, ++off) {
            color = data[off] >> 4;
            BMP_SET(RGB(color_map[(color * 4) + 2],
                        color_map[(color * 4) + 1],
                        color_map[(color * 4) + 0]));
            TEST_J;
            printf("%d: %d %d\n", i, j, j % info.width);
