...
//  roguelike
//
//  Created by Rory B. Bellows on 30/06/2018.
...
static void impl_tss_dtor_invoke() {
  for (int i = 0; i < EMULATED_THREADS_TSS_DTOR_SLOTNUM; i++) {
    if (impl_tss_dtor_tbl[i].dtor) {
 ...
 */
#if !defined(THREADS_OSX) && !defined(__CYGWIN__)
...
void *impl_thrd_routine(void *p) {
  ...
  free(p);
  return (void*)(uintptr_t)pack.func(pack.arg);
}
