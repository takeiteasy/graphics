...
void on_error(enum graphics_error type, const char* msg, const char* file, const char* func, int line) {
  fprintf(stderr, "ERROR ENCOUNTERED: %s\nFrom %s, in %s() at %d\n", msg, file, func, line);
  abort();
}

void on_keyboard(void* _, enum key_sym sym, enum key_mod mod, bool down) {
  if (sym == KB_KEY_ESCAPE)
    running = false;
  printf("keyboard: %d is %s\n", sym, (down ? "down" : "up"));
}

void on_mouse_btn(void* _, enum button btn, enum key_mod mod, bool down) {
...
void on_scroll(void* _, enum key_mod mod, float dx, float dy) {
  printf("scroll: %f %f\n", dx, dy);
}
...
int main(int argc, const char* argv[]) {
  ...
  bmp(&img, "/Users/roryb/git/graphics/tests/bmp/g/rgb32.bmp");
